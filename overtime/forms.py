...
class AvailabilitySheetForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['mon_one'] = forms.ChoiceField(label="First Monday:", widget=forms.Select, choices=monday_query_choices)
        self.fields['mon_two'] = forms.ChoiceField(label="Second Monday:", widget=forms.Select, choices=monday_query_choices)
        ...
        self.fields['tue_one'] = forms.ChoiceField(label="First Tuesday:", widget=forms.Select, choices=tuesday_query_choices)
        self.fields['tue_two'] = forms.ChoiceField(label="Second Tuesday:", widget=forms.Select, choices=tuesday_query_choices)
        ...
        self.fields['wed_one'] = forms.ChoiceField(label="First Wednesday:", widget=forms.Select, choices=wednesday_query_choices)
        self.fields['wed_two'] = forms.ChoiceField(label="Second Wednesday:", widget=forms.Select, choices=wednesday_query_choices)
        ...
        self.fields['thurs_one'] = forms.ChoiceField(label="First Thursday:", widget=forms.Select, choices=thursday_query_choices)
        self.fields['thurs_two'] = forms.ChoiceField(label="Second Thursday:", widget=forms.Select, choices=thursday_query_choices)
        ...
        self.fields['fri_one'] = forms.ChoiceField(label="First Friday:", widget=forms.Select, choices=friday_query_choices)
        self.fields['fri_two'] = forms.ChoiceField(label="Second Friday:", widget=forms.Select, choices=friday_query_choices)
        ...
        self.fields['sat_one'] = forms.ChoiceField(label="First Saturday:", widget=forms.Select, choices=saturday_query_choices)
        self.fields['sat_two'] = forms.ChoiceField(label="Second Saturday:", widget=forms.Select, choices=saturday_query_choices)
        ...
        self.fields['sun_one'] = forms.ChoiceField(label="First Sunday:", widget=forms.Select, choices=sunday_query_choices)
        self.fields['sun_two'] = forms.ChoiceField(label="Second Sunday:", widget=forms.Select, choices=sunday_query_choices)
...
class NonScheduledOvertimeRequestForm(forms.ModelForm):
    nsot_date = forms.DateField(label="Please enter the date of your Non_Scheduled Overtime Request:", required=True, widget=forms.DateInput(attrs={'type': 'date'}))
    nsot_start_time = forms.TimeField(label="Please enter the start time:", required=True, widget=forms.TimeInput(attrs={'type': 'time'}))
    nsot_end_time = forms.TimeField(label="Please enter the end time:", required=True, widget=forms.TimeInput(attrs={'type': 'time'}))
    nsot_task = forms.CharField(label="Please outline the task that required the extra overtime:", required=True, widget=forms.Textarea())
    directed_by = forms.ModelChoiceField(label="Please identify the Supervising Officer that directed you on the task:", widget=forms.Select, queryset=Account.objects.filter(is_staff=True))
...
class AllowancesRequestForm(forms.ModelForm):
    allow_req_date = forms.DateField(label="Please select the date of your allowance request:", required=True, widget=forms.DateInput(attrs={'type': 'date'}))
    allow_req_task = forms.CharField(label="Please outline the task for which you are requesting the allowance for:", required=True, widget=forms.Textarea())
    claiming_breakfast_allowance = forms.BooleanField(label="Please select this box if you are claiming a Breakfast Allowance:", required=False, widget=forms.CheckboxInput)
    claiming_dinner_allowance = forms.BooleanField(label="Please select this box if you are claiming a Dinner Allowance:", required=False, widget=forms.CheckboxInput)
    claiming_tea_allowance = forms.BooleanField(label="Please select this box if you are claiming a Tea Allowance:", required=False, widget=forms.CheckboxInput)
    claiming_plain_clothes_allowance = forms.BooleanField(label="Please select this box if you are claiming the Plain Clothes Allowance:", required=False, widget=forms.CheckboxInput)
    claiming_food_for_prisoner_expense = forms.BooleanField(label="Please select this box if you are claiming a Prisoner's Meal Allowance:", required=False, widget=forms.CheckboxInput)
    food_for_prisoner_amount = forms.FloatField(label="Please enter the cost of the Prisoner's Meal Allowance:", required=False, widget=forms.NumberInput)
...
class RejectAllowanceRequestForm(forms.ModelForm):
    reason_allowance_req_rejected = forms.CharField(label="Please outline why this allowance request is being rejected:", required=True, widget=forms.Textarea())
...
class RejectNSOTForm(forms.ModelForm):
    reason_nsot_rejected = forms.CharField(label="Please outline why this Non-Scheduled Overtime Request is being rejected:", required=True, widget=forms.Textarea())
...
class AssignOvertimeDateForm(forms.Form):
    date_for_assignment = forms.DateField(label="Please select the date were staff are needed for overtime:", required=True, widget=forms.DateInput(attrs={'type': 'date'}))
...
class AssignRecallStaffForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['selected_date'] = forms.DateField(label="Date for Recall:", disabled=True)
        ...
        if selected_date.strftime("%A") == "Monday":
            ...
            if length_mon_query == 0:
                ...
                self.fields['officers_available'] = forms.ChoiceField(label="Please select available officer:", widget=forms.Select, disabled=True, help_text="No staff have made themselves available for " + str(selected_date) + ".")
            else:
                mon_off_query_choices = [('', 'Select available staff member')] + [(id.avail_sheet_off_id, id.avail_sheet_off_id) for id in mon_query]
                self.fields['officers_available'] = forms.ChoiceField(label="Please select available officer:", widget=forms.Select, choices=mon_off_query_choices)
            ...
            if length_tue_query == 0:
                ...
                self.fields['officers_available'] = forms.ChoiceField(label="Please select available officer:", widget=forms.Select, choices=tue_off_query_choices)
            ...
            if length_wed_query == 0:
                ...
                self.fields['officers_available'] = forms.ChoiceField(label="Please select available officer:", widget=forms.Select, choices=wed_off_query_choices)
            ...
            if length_thurs_query == 0:
                ...
                self.fields['officers_available'] = forms.ChoiceField(label="Please select available officer:", widget=forms.Select, choices=thurs_off_query_choices)
            ...
            if length_fri_query == 0:
                ...
                self.fields['officers_available'] = forms.ChoiceField(label="Please select available officer:", widget=forms.Select, choices=fri_off_query_choices)
            ...
            if length_sat_query == 0:
                ...
                self.fields['officers_available'] = forms.ChoiceField(label="Please select available officer:", widget=forms.Select, choices=sat_off_query_choices)    
            ...
            if length_sun_query == 0:
                ...
                self.fields['officers_available'] = forms.ChoiceField(label="Please select available officer:", widget=forms.Select, choices=sun_off_query_choices)
        
        if no_staff_available_for_selected_date == True:
            self.fields['assign_shift'] = forms.ChoiceField(label="Please select a shift for the recall:", widget=forms.Select, disabled=True)
        else:
            self.fields['assign_shift'] = forms.ModelChoiceField(label="Please select a shift for the recall:", widget=forms.Select, queryset=Shift.objects.all())
...
class AssignRequireStaffForm(forms.Form):
   def __init__(self, *args, **kwargs):
        ...
        self.fields['selected_require_date'] = forms.DateField(label="Date for Requirement:", disabled=True)
        ...
        self.fields['officers_for_require'] = forms.ChoiceField(label="Please select an officer to compel them on this date:", widget=forms.Select, choices=off_to_require_query_choices)
        
        self.fields['assign_require_shift'] = forms.ModelChoiceField(label="Please select a shift for the requirement:", widget=forms.Select, queryset=Shift.objects.all())
...
class ShortTermAvailabilityForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['st_availability_date'] = forms.ChoiceField(label="Please select a date you wish to make yourself available in the short-term:", widget=forms.Select, choices=short_term_availability_choices)
...
class AssignShortTermOTDateForm(forms.Form):
    date_for_st_assignment = forms.DateField(label="Please select a date for the Short Term Recall:", required=True, widget=forms.DateInput(attrs={'type': 'date'}))
...
class AssignShortTermRecallStaffForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['selected_st_date'] = forms.DateField(label="Date for Short-Term Recall:", disabled=True)
        ...
        if length_st_query == 0:
            no_staff_available_for_selected_date = True
            self.fields['officers_available'] = forms.ChoiceField(label="Please select an Officer to Recall:", widget=forms.Select, disabled=True, help_text="No staff have made themselves available for " + str(selected_st_date) + ".")
        else:
            ...
            self.fields['officers_available'] = forms.ChoiceField(label="Please select an Officer to Recall:", widget=forms.Select, choices=st_off_query_choices)
...
class AssignShortTermRequireStaffForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['selected_st_require_date'] = forms.DateField(label="Date for Requirement:", disabled=True)
        ...
        off_to_require_query_choices = [('', 'select staff member to require')] + [(id.roster_officer_id, id.roster_officer_id) for id in officers_that_can_be_required]
        self.fields['officers_for_require'] = forms.ChoiceField(label="Please select an Officer to Require", widget=forms.Select, choices=off_to_require_query_choices)
